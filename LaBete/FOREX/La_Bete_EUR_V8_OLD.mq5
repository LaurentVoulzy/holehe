...
//| PARAMÈTRES INPUTS                                                 |
//+------------------------------------------------------------------+
input group "=== CONFIGURATION EUR/USD ==="
input double   RiskPercent = 0.3;           // Risque par trade (%)
input int      MagicNumber = 666001;        // Magic Number EUR
input string   TradeComment = "LaBete_EUR"; // Commentaire
...
input int      SL_MaxPips = 80;             // SL maximum EUR (pips)
input double   ATR_Multiplier_SL = 1.5;     // ATR × 1.5 pour EUR
...
int OnInit()
{
    ...
    // Vérifier qu'on est sur EUR/USD
    if(_Symbol != "EURUSD")
    {
        Print("⚠️ ATTENTION: Ce bot est optimisé pour EUR/USD uniquement!");
...
void AnalyzeMarket()
{
    Print("🔍 Analyse EUR/USD en cours...");
...
{
    Print("════════════════════════════════════════");
    Print("📈 OUVERTURE POSITION EUR/USD");
    Print("════════════════════════════════════════");
